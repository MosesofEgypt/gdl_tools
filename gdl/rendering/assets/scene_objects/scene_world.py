...
from ..animation import TextureAnimation
...
class SceneWorld(SceneObject):
    ...
    @property
    def collision_grid(self): return self._coll_grid
    @property
    ...
    @property
    def world_texmods(self): return dict(self._world_texmods)
    @property
    ...
    def optimize_node_graph(self):
        ...
        dyn_objects_nodepath = panda3d.core.NodePath(self.dynamic_objects_node)

        tex_anims = self.world_texmods
        ...
        for tex_anim in tex_anims.values():
            ...
            for lm_texture in lm_textures.values():
                ...
                combined_model = Model(name=f"__texmod_{tex_anim.tex_name}_{lm_name}model")
        ...
        dyn_objects_nodepath.flatten_strong()

        self.clean_orphaned_world_objects()

        for world_object in self.node_world_objects.values():
            for model in world_object.node_models.values():
        ...
        for child in self.p3d_nodepath.findAllMatches('**'):
            ...
            if isinstance(node, panda3d.core.GeomNode) and node.getNumGeoms() > 1:
                #print(panda3d.core.NodePath(node))
    ...
    def remove_world_texmod(self, tex_name):
        tex_name = tex_name.upper().strip()
        self._world_texmods.pop(tex_name, None)

    def add_world_texmod(self, texmod):
        if not isinstance(texmod, TextureAnimation):
            raise TypeError(f"Texmod object must be of type TextureAnimation, not {type(texmod)}")

        self._world_texmods[texmod.tex_name] = texmod
